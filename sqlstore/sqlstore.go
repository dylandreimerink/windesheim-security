...
//Original author: jstemmer
//Original url: https://github.com/jstemmer/sqlstore
...
func (s *SQLStore) load(session *sessions.Session) (exists bool, err error) {
	...
	row := s.db.QueryRow("SELECT data, updated_at FROM sessions WHERE id = ? LIMIT 1", session.ID)
...
func (s *SQLStore) save(session *sessions.Session) error {
	...
	var query string
	if session.IsNew {
		query = "INSERT INTO sessions(id, data, created_at, updated_at) VALUES (?, ?, NOW(), NOW())"
		_, err = s.db.Exec(query, session.ID, buf.Bytes())
	} else {
		query = "UPDATE sessions SET data=?, updated_at=NOW() WHERE id=?"
		_, err = s.db.Exec(query, buf.Bytes(), session.ID)
	}
